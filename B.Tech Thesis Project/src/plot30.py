...
with open('authorid_30pubs.txt','r') as fp:
    authorid_30pubs=json.loads(fp.read(),encoding='utf-8')

with open('authorid_30years.txt','r') as fp:
    authorid_30years=json.loads(fp.read(),encoding='utf-8')
...
authorid_30pubs=convert(authorid_30pubs)
authorid_30years=convert(authorid_30years)
authorid_author=convert(authorid_author)



authorid_30pubs={int(k):[int(i) for i in v] for k,v in authorid_30pubs.items()}
authorid_30years={int(k):[int(i) for i in v] for k,v in authorid_30years.items()}
authorid_author={int(k):str(v) for k,v in authorid_author.items()}


total=len(authorid_30pubs)
...
for key in authorid_30pubs:
    #print(key)
    citations=authorid_30years[key]
    pubs=authorid_30pubs[key]
    cumu=[]
    if pubs[21]==0:
        pubs[21]=1
    uu=0
...
with open('peaklate30.txt','w') as fp:
    json.dump(peaklate,fp,indent=4)

with open('peakint30.txt','w') as fp:
    json.dump(peakint,fp,indent=4)

with open('peakmul30.txt','w') as fp:
    json.dump(peakmul,fp,indent=4)

with open('monicr30.txt','w') as fp:
    json.dump(monicr,fp,indent=4)

with open('mondcr30.txt','w') as fp:
